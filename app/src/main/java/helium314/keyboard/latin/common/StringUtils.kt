...
private fun isPartOfCompositionForScript(codePoint: Int, spacingAndPunctuations: SpacingAndPunctuations, script: String) =
    spacingAndPunctuations.isWordConnector(codePoint) // We always consider word connectors part of compositions.
        // Otherwise, it's part of composition if it's part of script or an accidental digit inside words, and not a separator.
        || (!spacingAndPunctuations.isWordSeparator(codePoint) && (ScriptUtils.isLetterPartOfScript(codePoint, script) || Character.isDigit(codePoint)))
